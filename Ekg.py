...
# Ścieżka dostosowana
path_inp =r"C:\Users\Amelia\Desktop\spyder"
...
fiolet      = "#9C27B0" # Nowy kolor
zielony     = "#4CAF50" # Nowy kolor
...
st.markdown(f'<hr style="margin-top: 10px;height:5px; border:none; background-color:{fiolet};" />', unsafe_allow_html=True)
...
with col3:
    with st.container(border=True):
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=fiolet, width=1)
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=zielony, width=2)
        ))
...
with col_hrv:
    st.markdown(f"""<p style="font-size: 18px; font-weight: bold; color: {fiolet};">Identyfikacja załamków R i tworzenie szeregu RR</p>""", unsafe_allow_html=True)
    st.markdown(f"""<hr style="margin-top: -10px; height:5px; border:none; background-color:{fiolet};" />""", unsafe_allow_html=True)
    ...
    with c_left:
        ...
        st.markdown(f'<p style="font-size: 14px; color: {fiolet}; text-align: center;">Identyfikacja załamków R</p>', unsafe_allow_html=True)
        ...
        fig_zoom = go.Figure()
        fig_zoom.add_trace(go.Scatter(x=df_zoom['czas'], y=df_zoom['ecg_filtered'], line=dict(color=zielony, width=2)))
        fig_zoom.add_trace(go.Scatter(x=df_zoom['czas'].iloc[peaks_zoom], y=df_zoom['ecg_filtered'].iloc[peaks_zoom], mode='markers', marker=dict(color=fiolet, size=10)))
    ...
    with c_right:
        ...
        st.markdown(f'<p style="font-size: 14px; color: {fiolet}; text-align: center;">Szereg RR (ms)</p>', unsafe_allow_html=True)
        ...
        if len(rr_intervals) > 0:
            ...
            fig_rr.add_trace(go.Scatter(
                ...
                y=rr_intervals, 
                mode='lines+markers',
                line=dict(color=zielony, width=1),
                marker=dict(size=4)
            ))
...
with col_hist:
    st.markdown(f'<p style="font-size: 18px; font-weight: bold; color:{fiolet};">Histogram</p>', unsafe_allow_html=True)
    st.markdown(f"""<hr style="margin-top: -10px; height:5px; border:none; background-color:{fiolet};" />""", unsafe_allow_html=True)
    # Histogram
    if len(rr_intervals) > 0:
        fig_hist = px.histogram(x=rr_intervals, nbins=20, color_discrete_sequence=[zielony], labels={'x': 'Odstęp RR [ms]'})
...
st.markdown(f"""<p style="font-size: 18px; font-weight: bold; color: {fiolet};">Empiryczna Dekompozycja Modalna (EMD)</p>""", unsafe_allow_html=True)
st.markdown(f"""<hr style="margin-top: -10px; height:5px; border:none; background-color:{fiolet};" />""", unsafe_allow_html=True)
...
for i, name in enumerate(names):
        ...
        fig_emd.add_annotation(
            dict(
                ...
                textangle=0,
                xanchor='right',
                font=dict(size=12, color=fiolet)
            )
        )
...
fig_emd.update_xaxes(
        ...
        title_font=dict(color=zielony, size=14), # Kolor napisu "Time (samples)"
...
st.markdown(f"""<p style="font-size: 18px; font-weight: bold; color: {fiolet};">Usuwanie modulacji oddechowej (Prostowanie EKG)</p>""", unsafe_allow_html=True)
st.markdown(f"""<hr style="margin-top: -10px; height:5px; border:none; background-color:{fiolet};" />""", unsafe_allow_html=True)
...
if n_imfs >= 2:
        ...
        fig_clean.add_trace(go.Scatter(
            ...
            line=dict(color=zielony, width=1.5)
        ))
...
st.markdown(f"""<p style="font-size: 18px; font-weight: bold; color: {fiolet};">Analiza Widmowa (FFT) - Przed i Po</p>""", unsafe_allow_html=True)
st.markdown(f"""<hr style="margin-top: -10px; height:5px; border:none; background-color:{fiolet};" />""", unsafe_allow_html=True)
